...
const TRANSLATIONS = {
  en: {
    ...
    sendOtp: "Send OTP", otpLabel: "Enter OTP", otpPlaceholder: "Enter 6-digit OTP",
    verifyOtp: "Verify & Login", profileTitle: "My Profile", name: "Name",
    ...
    otpSent: "OTP sent to", healthy: "Plant looks healthy!",
  ...
  kn: {
    ...
    sendOtp: "OTP ಕಳಿಸಿ", otpLabel: "OTP ನಮೂದಿಸಿ", otpPlaceholder: "6 ಅಂಕಿ OTP ನಮೂದಿಸಿ",
    verifyOtp: "ಪರಿಶೀಲಿಸಿ ಮತ್ತು ಲಾಗಿನ್", profileTitle: "ನನ್ನ ಪ್ರೊಫೈಲ್", name: "ಹೆಸರು",
    ...
    otpSent: "OTP ಕಳಿಸಲಾಗಿದೆ", healthy: "ಸಸ್ಯ ಆರೋಗ್ಯಕರವಾಗಿದೆ!",
  ...
  hi: {
    ...
    sendOtp: "OTP भेजें", otpLabel: "OTP दर्ज करें", otpPlaceholder: "6 अंकों का OTP दर्ज करें",
    verifyOtp: "सत्यापित करें और लॉगिन", profileTitle: "मेरी प्रोफाइल", name: "नाम",
    ...
    otpSent: "OTP भेजा गया", healthy: "पौधा स्वस्थ है!",
...
export default function App() {
  ...
  const handleSendOtp = () => {
    if (mobile.length === 10) { setOtpSent(true); setError(""); }
    else setError("Please enter a valid 10-digit mobile number");
  };

  const handleVerifyOtp = () => {
    if (otp.length === 6) { setLoggedIn(true); setUser(u => ({ ...u, mobile })); setPage("home"); setError(""); }
    else setError("Please enter valid 6-digit OTP");
  };
      ...
      const resp = await fetch("https://api.anthropic.com/v1/messages", {
        ...
        body: JSON.stringify({
          ...
          messages: [{ role: "user", content: [
            ...
            { type: "text", text: `You are an expert agricultural plant pathologist. Analyze this plant image. Respond ONLY in JSON, no extra text. Use ${langName} for disease name, solution, prevention. JSON format: {"hasDisease": true/false, "diseaseName": "name", "crop": "crop name", "severity": "Low/Medium/High/Severe", "confidence": 85, "solution": "treatment steps", "prevention": "prevention tips"}` }
          ]}]
        })
      });
    ...
    if (page === "login") return (
      <div style={styles.page}>
        ...
        <div style={{ background: "white", borderRadius: "24px 24px 0 0", marginTop: -24, padding: "28px 24px", minHeight: "60vh" }}>
          ...
          {!otpSent && <button style={{ ...styles.btn, width: "100%", padding: 14 }} onClick={handleSendOtp}>{t.sendOtp}</button>}
          {otpSent && <>
            <div style={{ background: "#e8f5e9", borderRadius: 10, padding: "10px 16px", marginBottom: 14, color: "#2d8a47", fontSize: 14, fontWeight: 600 }}>✅ {t.otpSent} +91 {mobile}</div>
            <label style={{ display: "block", fontWeight: 600, color: "#1a5c2a", marginBottom: 8 }}>{t.otpLabel}</label>
            <input style={{ ...styles.input, marginBottom: 14 }} type="tel" maxLength={6} value={otp} onChange={e => setOtp(e.target.value.replace(/\D/, "").slice(0, 6))} placeholder={t.otpPlaceholder} />
            <button style={{ ...styles.btn, width: "100%", padding: 14 }} onClick={handleVerifyOtp}>{t.verifyOtp}</button>
          </>}
      ...
      <div style={{ padding: 16 }}>
        ...
        {page === "profile" && (
          <div>
            ...
            <button onClick={() => { setLoggedIn(false); setPage("home"); setUser({ name: "", mobile: "", location: "", crops: "" }); setMobile(""); setOtp(""); setOtpSent(false); }}
